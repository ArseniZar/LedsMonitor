<template>
    <div
        ...
        <img :src="device.imgIcon" alt="Check" class="h-6 w-6" />
        <span class="flex-1 text-lg font-bold text-white/90 ">{{ device.title }}</span>
        <button @click="deleteDevice">
            <img :src="'img/icons8-close-52.png'" alt="Check" class="h-5 w-5" />
        </button>
    </div>
</template>
